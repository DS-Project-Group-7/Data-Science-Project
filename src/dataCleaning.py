...
def main(dataFile):
    ...
    for feature in CategoricalColumnsDict:
        cleanDataDf[feature]= fuseCategColumns(originalDataDf, CategoricalColumnsDict[feature], feature)

    for feature in OrdinalColumnsDict:
        cleanDataDf[feature]= fuseOrdinalColumns(originalDataDf, OrdinalColumnsDict[feature], feature)
...
def fuseOrdinalColumns(originalDf,orderedIndexList, colName):
    ...
    for index in orderedIndexList[1:]:
        level+=1
        researchInOrigin= (originalDf.iloc[:,index]!=0) & (originalDf.iloc[:,index]!="n/a")
        newColumn.loc[researchInOrigin & (newColumn[colName]!=0), colName]=\
            np.floor((newColumn.loc[researchInOrigin & (newColumn[colName]!=0), colName] + level)/2)
...
def parseArguments():
    ...
    # Optional arguments
    parser.add_argument("-d", "--data", help="Csv data file path", type=str, default='../Cleaning_data/ManuallyCleanedData.xlsx')
