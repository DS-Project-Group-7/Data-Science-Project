...
colnames(paint_support_df) <- c('collection', 'planar', 'warped', 'indentations', 'good tension', 
...
for (i in 2:21) {
  for (j in 2:21) {
    if (i == j) {
      ...
      #                                paint_support_df[,i]))
      #three_way <- glm(Freq ~ factor(Var3)*factor(Var1) + factor(Var2)*factor(Var1), family = poisson, data = test_df)
