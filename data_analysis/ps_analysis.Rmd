...
colnames(paint_support_df) <- c('planar', 'warped', 'indentations', 'good_tension', 'holes', 'loose',
                                'tears', 'taut', 'surface_dirt', 'mould', 'staining', 'corner_distortions',
                                'top_distortions', 'bottom_distortions', 'overall_distortions',
                                'insect_damage', 'rust_stains', 'deformation_around_tacks_staples',
                                'tears_around_tacks_staples', 'loss_of_tacks_insecure_support')
...
count <- 1
for (i in 1:20) {
  if (i == length(colnames(paint_support_df))) break
  for (j in (i+1):20) {
...
as.data.frame(cbind(attr1, attr2, pval))
```
